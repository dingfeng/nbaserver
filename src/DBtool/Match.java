...
public class Match 
{
   ...
   private final static int match_num = 150;
  public Match(String path)
   {
		match_data = new MatchData(path);
	...
	private void dealWithOneMatch(MatchesPO match)
	{
		...
		if (!team_map.containsKey(team1.getName().hashCode()))
		{
			team_map.put(team1.getName().hashCode(), new TeamQueue(match_num,team1.getName()));
		}
		if (!team_map.containsKey(team2.getName().hashCode()))
		{
			team_map.put(team2.getName().hashCode(), new TeamQueue(match_num,team2.getName()));
		}
